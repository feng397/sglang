...
IndexCache reuses C4 indexer topk results across layers. This can improve long-context serving throughput with a small accuracy tradeoff. To try it from the [Playground above](#playground), open the **IndexCache** card and select `index_topk_freq=4`. The equivalent command-line override is:
...
Use this option for long-context performance experiments. Keep it disabled for strict accuracy comparisons unless you have validated the target workload.
